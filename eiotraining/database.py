...
def generate_id():
	...
	while(1):
		id = mc.gets('cur_id')
		if id == None:
			cursor = db_execute('SELECT value FROM variables WHERE name = "cur_id"')
			...
			if(id % 1000 == 0):
				db_execute('UPDATE variables SET value = %s WHERE name = "cur_id"', id + 1000)
				db.commit()
			return id
...
def signup(user, password, email):
	...
	"""
	cursor = db_execute('SELECT COUNT(*) FROM users WHERE user = %s', user)
	...
	id = generate_id()
	db_execute('INSERT INTO users VALUES(%s, %s, %s, %s, %s, %s)', (user, hash, salt, id, email, 0) )
	db.commit()
...
def create_post(title, content, user_id):
	...
	id = generate_id()
	db_execute("INSERT INTO news VALUES (%s, %s, %s, %s, %s)", (title, content, id, datetime.datetime.now(), user_id))
	db.commit()
	mc.delete('posts')
...
def get_num_posts():
	...
	if(num_posts == None):
		cursor = db_execute('SELECT COUNT(*) FROM news')
...
def get_navlinks(parent):
	...
	if(navlinks == None):
		cursor = db_execute('SELECT * FROM navlinks WHERE parent = %s', parent)
...
def get_navlink(id):
	...
	cursor = db_execute('SELECT * FROM navlinks WHERE id = %s', id)
...
def create_navlink(parent, type, description):
	...
	db_execute('INSERT INTO navlinks VALUES (%s, %s, %s, %s)', (parent, id, type, description) )
	if(type == variables.navlink_tutorial or type == variables.navlink_problem):
		db_execute('INSERT INTO statements VALUES (%s, %s, %s)', (id, "", type))
	db.commit()
	mc.delete('navlinks:'+str(parent))

def delete_navlink(navlink):
	"""Deletes a single navlink"""
	db_execute('DELETE FROM navlinks WHERE id = %s', navlink[1])
	if(navlink[2] == variables.navlink_tutorial or navlink[2] == variables.navlink_problem):
		db_execute('DELETE FROM statements WHERE parent = %s', navlink[1])
...
def get_statement(parent):
	...
	if(statement == None):
		cursor = db_execute('SELECT * FROM statements WHERE parent = %s', parent)
...
def edit_statement(parent, statement_text):
	...
	cursor = db_execute('UPDATE statements SET statement = %s WHERE parent = %s', (statement_text, parent) )
	db.commit()
